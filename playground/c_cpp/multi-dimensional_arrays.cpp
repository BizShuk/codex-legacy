    ...
    for (i = 0; i < row; i++)
        for (j = 0; j < col; j++)
            if (*(a + i * cols + j) > highgrade)  // How does this line work?  
                // Ans: each student has problemSets 10. It maens each one has 10 problem score and each score has 4 bytes(1 index long) , So cols is the space of each student
